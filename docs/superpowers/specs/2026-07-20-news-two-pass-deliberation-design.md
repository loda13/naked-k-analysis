...
- 如果只返回一个由类型或 capability 元数据明确标注的可用文本聊天模型，自动采用；不得把 embedding、rerank、图像、音频或审核模型当作聊天模型。
- 如果返回多个模型，不猜测“最佳模型”；CLI 输出可用模型 ID，并要求通过环境变量或 `--news-model` 明确选择。
- 如果模型条目没有足够的类型/capability 元数据，也不猜测；展示 ID 并要求明确选择。
- 模型发现的网络、HTTP、解析或空列表故障不得中断纯技术报告；只有多个/不明确模型需要用户选择时才以配置错误退出。
...
`观望` 与 `回避` 不占用新的账户风险预算；其综合风险计划的建议总仓位和有效账户风险均为 `0`，避免零仓位动作在组合层形成虚假风险暴露。
...
- 第一轮有效返回但 `data_quality=insufficient`：保留消息结论，跳过第二轮，最终动作使用技术动作。
- 第一轮请求、解析或验证失败：不执行第二轮，最终动作使用技术动作。
- 第一轮成功、第二轮失败：报告仍展示消息结论，但最终动作使用技术动作。
- 第二轮输出非法动作、篡改技术动作或引用不存在证据：视为无效并退回技术动作。
- 重新生成价格计划失败：保留第二轮的模型动作、冲突分析、理由和证据供阅读，但最终动作与所有执行字段退回技术动作，并记录具体回退原因。
- 组合保护执行异常：撤销本轮所有部分组合改写，保留各标的在组合保护前已经有效的综合结果，记录脱敏 warning，并继续生成 journal 与报告。
