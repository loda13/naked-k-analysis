...
- **覆盖范围**：全市场快讯摘要（非按标的检索）
- **时效性**：分钟级，实测每次返回最新 20 条
- **接入方式**：`akshare.stock_info_global_sina()`
  - 注意：不存在 `stock_news_sina()`；`stock_news_em()` 是东财，已由
...
新浪滚动快讯的接入把中文快讯从「天级滞后」拉到了分钟级，这是「小米新车发布」
这类事件的主要捕获路径。仍需注意：新浪是全市场摘要，按标题归属，
命中率取决于快讯标题是否点名标的；要进一步提高召回，需要财联社这类
按标的检索的专业源。
