...
- `naked_k_news_sec.py` — SEC EDGAR Form 8-K filings for US-listed material events. Resolving a ticker costs a ~2MB `company_tickers.json` index download, so tickers carrying a non-US exchange suffix (`_NON_US_SUFFIXES`) **return empty before any network call** — the default pool is all-HK and would otherwise pay four round trips per run for a guaranteed `None`. OTC ADRs (TCEHY, XIACF) have no CIK and are not covered.
