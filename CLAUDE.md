...
- `naked_k_news_enhanced.py` — **[v3.2.0]** Multi-source intelligent merger with relevance scoring, quality weighting (Finnhub 3.0x, AkShare 2.0x, Google 1.0x, Yahoo 0.5x), word-boundary matching, and differentiated time windows. Improves relevant-news ratio from 0% → 71% (Xiaomi 1810.HK live test). Per-provider relevance gate in `_passes_relevance_gate`: Finnhub bypasses it (already symbol-scoped), **AkShare requires a title match** because East Money mixes in market-wide capital-flow tables whose bodies list every ticker code *and* issuer name (so any body-derived score leaks them), everything else keeps the looser body threshold. Word-boundary matching is ASCII-only — `\b` can never match a 2–3 char CJK name, so 小米/腾讯 would otherwise score 0.
