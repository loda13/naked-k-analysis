...
- `naked_k_analysis.classify_market` and `naked_k_trade.classify_market` both reuse the canonical `naked_k_portfolio.classify_market`, which maps `.BJ` to `cn` and recognises crypto. `naked_k_portfolio` imports only `naked_k_config`, so this adds no cycle. Do not add a third copy of this rule.
